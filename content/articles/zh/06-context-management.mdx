...
```mermaid
flowchart TD
    A["tokenCountWithEstimation(messages)"] --> B{"最后一个 assistant<br/>有 usage 数据？"}
    B -->|是| C["获取 API usage<br/>精确 token 数"]
    B -->|否| D["对全部消息做<br/>粗略估算"]
    C --> E["对 usage 之后的<br/>新消息做粗略估算"]
    E --> F["精确数 + 粗略增量<br/>= 当前 token 总量"]
    D --> F
...
```mermaid
flowchart TD
    A["autoCompactIfNeeded<br/>每次 query loop 调用"] --> B{"DISABLE_COMPACT<br/>环境变量？"}
    B -->|是| Z["返回 wasCompacted: false"]
    B -->|否| C{"连续失败次数<br/>>= 3？"}
    C -->|是| Z
    C -->|否| D{"shouldAutoCompact<br/>token > 阈值？"}
    D -->|否| Z
    D -->|是| E["尝试 Session Memory<br/>Compaction"]
    E -->|成功| F["返回 wasCompacted: true<br/>重置失败计数"]
    E -->|失败/null| G["执行完整<br/>compactConversation"]
    G -->|成功| F
    G -->|异常| H["递增失败计数<br/>返回 wasCompacted: false"]
...
```mermaid
flowchart TD
    A["每次 API 请求前"] --> B["microcompactMessages<br/>第一层：轻量清理"]
    B --> C{"时间间隔 ><br/>阈值？"}
    C -->|是| D["Time-based MC<br/>清除旧工具结果内容"]
    C -->|否| E{"Cached MC<br/>可用？"}
    E -->|是| F["Cached MC<br/>通过 cache_edits 删除"]
    E -->|否| G["跳过 microcompact"]

    H["每次 query loop 迭代"] --> I["shouldAutoCompact<br/>token 超阈值？"]
    I -->|否| J["继续正常对话"]
    I -->|是| K["trySessionMemoryCompaction<br/>第二层：零 API 压缩"]
    K -->|成功| L["用 Session Memory<br/>替代旧消息"]
    K -->|失败| M["compactConversation<br/>第三层：LLM 完整摘要"]
    M -->|成功| N["用 LLM 摘要<br/>替代旧消息"]
    M -->|失败| O["记录失败<br/>下次重试或熔断"]

    P["用户输入 /compact"] --> Q["compactConversation<br/>手动触发完整压缩"]

...
>
> *（译：某个物种名与 excluded-strings.txt 中的模型代号哨兵字符串冲突。构建检查扫描的是产物（而非源码），因此在运行时拼接字符串可以避免字面量出现在构建产物中，同时保持检查对真正的模型代号仍然有效。）*
...
>
> *（译：使用本地日期而非 UTC——跨时区形成 24 小时的滚动浪潮。持续的 Twitter 讨论热度，而非 UTC 午夜的单点爆发，对 soul 生成的服务压力也更温和。）*

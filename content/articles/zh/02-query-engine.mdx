...
>
> *（译：必须在 submitMessage 内部的两次 processUserInputContext 重建之间持久化，但在每次 submitMessage 开始时清除，以避免在 SDK 模式下多轮对话中无限增长。）*
...
>
> *（译：如果进程在 API 响应之前被终止，会话记录中只剩下队列操作条目；getLastSessionLog 会过滤掉这些条目、返回 null，导致 --resume 失败并提示"No conversation found"。）*
